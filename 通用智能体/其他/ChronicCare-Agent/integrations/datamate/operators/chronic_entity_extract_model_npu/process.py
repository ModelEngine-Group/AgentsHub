...
class chronic_entity_extract_model_npu(Mapper):
    def execute(self, sample, params=None):
        ...
        if runtime.get("npu_available"):
            ...
            agreement = sum(row.get("entity_type") == row.get("npu_predicted_entity_type") for row in rows) / max(1, len(rows))
            ...
            if quality_gate["passed"]:
                for row in kept_rows:
                    ...
                    row["npu_provenance"] = {"model": model_path, "score": row.get("npu_entity_type_score"), "prediction": row.get("npu_predicted_entity_type")}
                _write_jsonl(entity_dir / "entities_raw.jsonl", kept_rows)
