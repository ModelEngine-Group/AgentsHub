...
![DKM 分层架构](../competition_submission/defense-package-final/evidence/figures/architecture_diagram.svg)

![数据—知识—洞察闭环](../competition_submission/defense-package-final/evidence/figures/dkm_workflow.svg)
...
![规划器算子数量：规则 vs LLM 增强](../competition_submission/defense-package-final/evidence/figures/planner_operator_comparison.svg)
...
![任务一数据质量提升](../competition_submission/defense-package-final/evidence/figures/task1_quality_improvement.svg)

**2.3 本地小模型与 LLM 增强（可选）**

任务一支持 QLoRA 微调规划 adapter（`data/training/model_output`，3 epochs，eval_loss 0.0005123）。训练与推理共用同一 system prompt，详见 `local_model_finetune.md`。LLM 增强路径（DeepSeek `deepseek-v4-flash`）已验证 `planner_mode=llm` 输出合法算子计划并完成清洗。

完整能力边界、复现命令与算子白名单见 [任务一：数据处理智能体](task1_data_agent.md)。
...
![封闭词表 vs OOV 抽取对比](../competition_submission/defense-package-final/evidence/figures/task2_oov_extraction.svg)
...
![任务二端到端流水线时延](../competition_submission/defense-package-final/evidence/figures/task2_pipeline_latency.svg)
...
![任务二知识图谱总览](../competition_submission/defense-package-final/evidence/figures/task2_kg_overview.svg)

![任务二实体类型分布](../competition_submission/defense-package-final/evidence/figures/task2_entity_types.svg)
...
![Neo4j 节点总览](../competition_submission/defense-package-final/evidence/screenshots/neo4j/01_nodes_overview.png)
...
![高血压关系表](../competition_submission/defense-package-final/evidence/screenshots/neo4j/02_hypertension_edges.png)
...
![高血压关系图](../competition_submission/defense-package-final/evidence/screenshots/neo4j/03_hypertension_graph.png)
...
![节点类型统计](../competition_submission/defense-package-final/evidence/screenshots/neo4j/04_type_distribution.png)

*说明：Symptom 8、Examination 6、Drug 5、Disease 4、Treatment 3，合计 26 节点，与 §3.3 离线图谱及在线读回一致。*

**3.5 LLM 增强（可选）**

LLM 增强路径已验证：配置 DeepSeek（`deepseek-v4-flash`）后，`planner=llm | LLM=active`，4 chunks processed by LLM，生成 39 triples，QA answered。本地小模型 NER adapter（`kg_model_output/final`，3 epochs，eval_loss 0.005754）同样验证 `local_model=active`。详见 `local_model_finetune.md`。同一批 4 条内置样例上，规则 demo 默认规模为 §3.3 的 29 edges；LLM 路径 triple 数更多，属可选增强模式，各场景完整对照见 §1.4。

完整 schema、能力边界、Neo4j 适配与复现命令见 [任务二：医疗知识图谱智能体](task2_kg_agent.md)。
...
![任务三 NL2SQL 三档准确率](../competition_submission/defense-package-final/evidence/figures/task3_nl2sql_accuracy.svg)
...
![实体分布](../competition_submission/defense-package-final/evidence/figures/entity_distribution.svg)

**关系类型分布**

![关系分布](../competition_submission/defense-package-final/evidence/figures/relation_distribution.svg)

**疾病关联网络（摘录）**

![疾病网络](../competition_submission/defense-package-final/evidence/figures/disease_network.svg)

**记录趋势**

![记录趋势](../competition_submission/defense-package-final/evidence/figures/record_trend.svg)

**ECharts 交互仪表盘（`task3_interactive_dashboard.html`）**

![任务三 ECharts 总览](../competition_submission/defense-package-final/evidence/screenshots/task3/01_interactive_dashboard_overview.png)

![任务三图谱可视化与 NL2SQL 证据](../competition_submission/defense-package-final/evidence/screenshots/task3/02_interactive_dashboard_graph_nl2sql.png)
...
LLM 增强路径已验证：配置 DeepSeek 后 `planner=llm | NL2SQL=llm | LLM=active`，`nl2sql=llm_generated`，5 rows，quality passed。本地小模型两个 adapter（`analysis_planning_model_output/final` eval_loss 0.06268、`analysis_nl2sql_model_output/final` eval_loss 0.001397）均验证 `NL2SQL=local_model_generated` 并返回正确结果。详见 `local_model_finetune.md`。

完整分析算子清单、NL2SQL 安全约束、可视化与复现命令见 [任务三：图谱驱动分析智能体](task3_analysis_agent.md)。
...
Windows 侧 Nexent/DataMate 容器在 WSL Ubuntu + Docker 中运行；任务 API 在 Windows PowerShell 启动。原生 Linux 部署时 Nexent 容器访问宿主机 API 需配置 `host.docker.internal` 或 `--docker-host auto`，详见 `online_integration.md`。部署边界见 `preparation.md`。
...
| 增强路径 | 三任务 LLM 增强（DeepSeek，复现命令见各任务文档中的 `deepseek-v4-flash` 配置）与本地小模型（4 个 QLoRA adapter，3 epochs）均已完成训练并通过验证，详见各任务文档与 `local_model_finetune.md` |
