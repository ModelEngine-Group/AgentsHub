...
> 按本仓库的目录规范，根目录的 `agent.json` 就是协作总控智能体的配置，直接在
> Nexent 的「智能体开发 → 导入」里上传它即可；另外三个在 `智能体源文件/` 下按需导入。
> 四者共用同一个 MCP 服务，导入前先按下方「三、快速开始」把服务跑起来。
...
MediGraphAgent/
├── README.md                      本文档
├── agent.json                     ← 平台导入入口：闭环协作总控智能体
│                                    （与 智能体源文件/collaboration_supervisor.json 相同）
